...
class mcmc_main:
    def __init__(self, city, per):
        ...
        #self.init0 = pd.to_datetime('2021-11-29')
        self.Init0= {'Davis (sludge)':pd.to_datetime('2021-07-01'),'UCDavis (sludge)':pd.to_datetime('2021-07-01'),
                     'Modesto':pd.to_datetime('2021-11-29'),
                     'Davis':pd.to_datetime('2021-11-29'),'UCDavis':pd.to_datetime('2021-11-29')}
        self.end = pd.to_datetime('2022-07-01')
        self.init0 = self.Init0[city]
        self.per = per
        self.size_window = 14
        ...
        #self.init_all, self.end_all = ['2021-11-29', '2022-07-01']

        #self.init_tr, self.end_tr = self.training_date_CDC[wave]
        #self.init_wave, self.end_wave = self.wave_date_CDC[wave]
        ...
        # self.Mu_b1 = {'UCDavis (sludge)':np.log(3000), 'Davis (sludge)':np.log(3000), 'Modesto':3000, 'Merced':4500,'Davis':500, 'UCDavis':500, 'all':4500} # Omicron
        # self.Sig_b1 = {'UCDavis (sludge)':np.log(1000)**2, 'Davis (sludge)':np.log(1000)**2, 'Modesto':1000**2,'Merced':1000**2, 'Davis':150, 'UCDavis':150}
    ...
    def read_data(self):
        ...
        city_data['NormalizedConc_s'] = city_data['NormalizedConc_crude'].rolling(window=10, min_periods=3, center=True).apply(lambda x: self.trim_fun(x))
        ...
        #city_data['pos_rate']=city_data['pos_rate'].rolling(window=7, center=True).mean()
    ...
    def predictive(self, x, X):
        mu = self.rate(x, X) # ebeta/(1+ebeta)
        a = mu * self.phi
        b = self.phi - a
...
"""
for i in range(29):
    mcmc = mcmc_main(city="Davis (sludge)", per=i)
    mcmc.RunMCMC()
"""

#city_data = mcmc.city_data
#mcmc.Data_mask.to_csv('data_Davis(sludge).csv')
